...
function lprd_404_page_content() {
	...
	$text_4 = esc_html__('Go back', 'leopard');
	echo sprintf('<div class="lprd-404-content"><h1>%1$s</h1><h2>%2$s</h2><p>%3$s</p><a class="lprd-default-btn lprd-404-btn" onclick="history.back()">%4$s<i class="fas fa-arrow-right"></i></a></div>', $text_1, $text_2, $text_3, $text_4);
...
if ( ! function_exists( 'lprd_posted_on' ) ){
	...
	function lprd_posted_on() {
		...
		$posted_on = sprintf(
			'<a class="lprd--post-date-text" href="%s" rel="bookmark">%s</a>',
...
if ( ! function_exists( 'lprd_posted_by' ) ) {
	...
	function lprd_posted_by() {
		...
		return sprintf(
			'<a class="lprd--post-author-text author vcard" href="%s">%s</a>',
			esc_url( $url ),
